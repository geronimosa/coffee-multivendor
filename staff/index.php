...
if ($authorized && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['order_id'])) {
    ...
    try {
        ...
        $paidPending = ($order['payment_status'] ?? 'unpaid') === 'paid' && in_array($current, ['pending','paid'], true);
        if ($target === 'confirm_payment' && $current === 'pending' && !empty($order['credit_card_payment'])) {
            $pdo->prepare("UPDATE orders SET payment_status='paid',payment_method='counter_card',paid_at=NOW(),status='paid' WHERE id=? AND restaurant_id=?")->execute([$orderId,$vendorId]);
            $valid = false;
        } else {
        $valid = ($target === 'preparing' && $paidPending)
            || ($target === 'complete' && $current === 'preparing')
            || ($target === 'collected' && $current === 'complete')
            || ($target === 'archived' && in_array($current, ['collected', 'cancelled'], true))
            || ($target === 'cancelled' && ($paidPending || in_array($current, ['preparing', 'complete'], true)));
        }
...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= e($vendor['name']) ?> staff</title><link rel="stylesheet" href="/assets/css/super-admin.css"><?php if ($authorized): ?><meta http-equiv="refresh" content="15"><?php endif; ?></head><body>
...
<?php else: ?><div class="actions"><div><h1>Fulfilment queue</h1><p class="muted">Payment-validated and counter-payment orders · refreshes every 15 seconds</p></div><a class="button secondary" href="/shop/<?= e($slug) ?>" target="_blank">Open customer shop</a></div>
...
<?php foreach ($orders as $order): $isPaid=$order['payment_status']==='paid';$isCounter=!$isPaid&&!empty($order['credit_card_payment']);$paymentClass=$isPaid?'paid':($isCounter?'counter':'unpaid');$paymentLabel=$isPaid?'PAID':($isCounter?'PAY AT COUNTER':'NO PAYMENT'); ?><section class="card"><div class="actions"><div><span class="badge"><?= e($tabs[$activeTab]['label']) ?></span> <span class="payment-badge <?= $paymentClass ?>"><?= $paymentLabel ?></span><h2>Order #<?= (int) $order['id'] ?> · <?= e($order['name']) ?></h2></div><strong>R<?= number_format((float) $order['total'], 2) ?></strong></div><p><?= e($order['items'] ?: 'No items') ?></p><form method="post" class="actions"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="order_id" value="<?= (int) $order['id'] ?>"><input type="hidden" name="tab" value="<?= e($activeTab) ?>"><?php if ($activeTab === 'pending' && $isPaid): ?><button name="status" value="preparing">Start preparing</button><button name="status" value="cancelled">Cancel</button><?php elseif ($activeTab === 'pending' && $isCounter): ?><button name="status" value="confirm_payment">Confirm payment received</button><button name="status" value="cancelled">Cancel</button><?php elseif ($activeTab === 'pending'): ?><span class="muted">Waiting for online payment</span><?php elseif ($activeTab === 'preparing'): ?><button name="status" value="complete">Mark ready</button><button name="status" value="cancelled">Cancel</button><?php elseif ($activeTab === 'ready'): ?><button name="status" value="collected">Mark collected</button><button name="status" value="cancelled">Cancel</button><?php elseif ($activeTab === 'collected'): ?><button name="status" value="archived">Archive</button><?php elseif ($order['status'] === 'cancelled'): ?><button name="status" value="archived">Archive</button><?php endif; ?></form></section><?php endforeach; ?>
