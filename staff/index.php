...
<?php if (!$authorized): ?><section class="card" style="max-width:480px;margin:auto"><h1>Portal sign in</h1><p class="muted">Enter the 10-character access key generated for this vendor.</p><?php if ($message): ?><div class="notice"><?= e($message) ?></div><?php endif; ?><?php if ($error): ?><div class="notice error"><?= e($error) ?></div><?php endif; ?><form method="post" id="staff-key-form"><input type="hidden" name="csrf_token" value="<?= e(csrf_token()) ?>"><input type="hidden" name="staff_key_login" value="1"><label for="staff_key">Access key</label><input id="staff_key" name="staff_key" required minlength="10" maxlength="10" pattern="[A-HJ-NP-Za-km-z2-9]{10}" autocomplete="one-time-code" autocapitalize="none" spellcheck="false"><p><button type="submit">Open vendor portal</button></p></form></section><script src="/assets/js/staff-key-link.js"></script>
