...
<section class="card"><h2>Messaging via Twilio</h2><p class="muted">Send transactional order updates by SMS or WhatsApp. SMS is recommended as the default. Saved credentials: <?= e($twilio['config_hint'] ?? 'Not configured') ?>.</p><div class="grid">
    ...
    <div><label for="twilio_fallback_channel">Additional channel</label><select id="twilio_fallback_channel" name="twilio_fallback_channel"><option value="none" <?=in_array($twilioFallback,['','none'],true)?'selected':''?>>Primary only</option><option value="sms" <?=$twilioFallback==='sms'?'selected':''?>>Also send SMS</option><option value="whatsapp" <?=$twilioFallback==='whatsapp'?'selected':''?>>Also send WhatsApp</option></select></div>
