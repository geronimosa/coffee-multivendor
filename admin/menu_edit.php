...
if($_SERVER['REQUEST_METHOD']==='POST'){
    require_csrf();
    $name=trim((string)($_POST['name']??''));$category=trim((string)($_POST['category']??''));$variants=json_decode($variantsJson,true);
    ...
    if(!$error){
        ...
        $pdo->prepare('UPDATE menu_items SET name=?,category=?,price=?,variant_options=? WHERE id=? AND restaurant_id=?')->execute([$name,$category,$price,json_encode($variants,JSON_THROW_ON_ERROR|JSON_UNESCAPED_UNICODE),$itemId,$restaurantId]);
